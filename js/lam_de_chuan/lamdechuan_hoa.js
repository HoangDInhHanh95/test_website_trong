...
function createExam() {
    ...
    const abcdQuestions = getRandomQuestions(questions.hoa.abcd, 18);
    const dungSaiQuestions = getRandomQuestions(questions.hoa.dung_sai, 4);
    const traLoiNganQuestions = getRandomQuestions(questions.hoa.tra_loi_ngan, 6);
...
function displayResults(answers) {
    ...
    <div class="results-header">
        ...
        <div class="nav-buttons">
            ...
            <a href="lamdechuan_hoa.html" class="nav-button">Làm Đề Khác</a>
        </div>
    </div>
...
`;
    ...
    dungSaiQuestions.forEach((q, index) => {
        ...
        resultsContent += `
                ...
                <ul>
                    ${Object.entries(q.solutions).map(([key, value]) => `
                        <li>${key}: ${value}</li>
                    `).join('')}
                </ul>
            </div>
    ...
    traLoiNganQuestions.forEach((q, index) => {
        ...
        resultsContent += `
            <div class="question result ${isCorrect ? 'correct' : 'incorrect'}">
                ...
                <p><strong>Giải thích:</strong> ${q.solution || 'Không có giải thích'}</p>
            </div>
        `;
    });
...
console.log("lamdechuan_hoa.js loaded");
...
let totalSeconds = 50 * 60; // 120 phút = 7200 giây
let timerInterval;
